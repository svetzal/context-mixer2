...
use crate::plugin_types::{Marketplace, MarketplaceEntry, PluginSource};
...
fn scan_marketplace_explicit_arrays(
    ...
    for path_str in &plugin.agents {
        let full_path = root.join(path_str);
        if !fs.exists(&full_path) {
            warnings.push(ScanWarning {
                message: format!("marketplace declares agent '{path_str}' but path does not exist"),
            });
            continue;
        }
    ...
    for path_str in &plugin.skills {
        let full_path = root.join(path_str);
        if !fs.exists(&full_path) {
            warnings.push(ScanWarning {
                message: format!("marketplace declares skill '{path_str}' but path does not exist"),
            });
            continue;
        }
        ...
        if !fs.exists(&skill_md) {
            warnings.push(ScanWarning {
                message: format!("marketplace declares skill '{path_str}' but SKILL.md is missing"),
            });
            continue;
        }
