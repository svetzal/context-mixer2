...
- **`cmx agent list` and `cmx doctor` no longer report a Codex-installed agent as unversioned or outdated.** The Codex TOML cmx generated carried no version at all, so the survey read the Codex copy as unversioned, collapsed "1.3.0 vs none" to none, and `list` reported `outdated` straight after `install`. The transform now preserves the source frontmatter (`metadata.version` included) in a `# ---`-fenced comment block Codex ignores, and `scan::extract_version_from_content` reads it back through `cmx_core::agent::preserved_frontmatter` when a file has no YAML fence. When a copy carries no version at all — a Codex agent installed by an older cmx — `doctor` falls back to the version recorded in its lock entry: the artifact is the primary truth, the lock is the backup. The same transform rewrite stops `description: |` block scalars installing as `description = "|"`.
- **`cmx skill promote` / `cmx agent promote` / `cmx skill sync` plans now report `+`/`-` in the diff direction.** The per-file counts were inverted: `+` showed lines that were about to *disappear* from the target and `-` showed lines about to *arrive*, so a purely additive promote of a 220-line skill that gained 44 lines rendered as `SKILL.md  modified (+0 -44)` — indistinguishable from a plan about to delete 44 lines. The cause was `diff::file_changes_between` returning `cmx diff`'s orientation (`+` is the installed copy) for callers whose first argument is the *before* side, with `display/util.rs` half-compensating by reading `change.added` for the `-` count on whole-file adds and deletes. `file_changes_between` now returns plan-oriented counts (`+` is what applying adds to the target, `-` what it removes) and the renderer reads them straight, matching `diff(1)` and every package manager. `cmx diff`'s own output is unchanged — it labels its direction explicitly. `cmx set activate`/`deactivate` were never affected; they report per-member actions and discarded paths, not line counts.
...
- **`cmx <kind> unadopt` no longer clears lock entries on platforms excluded via `cmx config platforms`.** Like the `promote` bug above, `unadopt` hand-rolled its own platform loop (`platform_iter::all()`) instead of the managed-platform allowlist, so it could untrack a home-provenance lock entry on a platform the user had explicitly excluded from management. Fixed by routing through the new shared `home_provenance` module.
...
- `cmx doctor` now renders a single `Platforms` column in both the default needs-attention table and `--all`, attributing each surveyed copy as `platform@version` (or `platform@unversioned`) instead of splitting versions from platforms or leaving external/source-less rows blank. The `Source` column now says `none` instead of `-`, and `cmx doctor --json` adds `locations[].platform` while preserving the existing `tools`, `version`, `versions`, and `locations` fields.
...
- `cmx init` now detects companion-skill drift from the on-disk copy's checksum instead of trusting the lockfile checksum alone, so in-place edits are surfaced even when `cmx-lock.json` still matches the bundled skill.
