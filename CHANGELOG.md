...
- **`cmx {skill,agent} diff` now prints the exact command to remediate.** When the installed copy differs from source, the output ends with a copy-pasteable next step — `cmx <kind> update <name>`, or `cmx <kind> update <name> --force` when the installed copy has local edits (detected from the lock checksum), with a one-line caveat that `--force` overwrites them. Previously you had to know which command (and flag) to reach for.
