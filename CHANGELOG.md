...
- `cmx list` and `cmx {skill,agent} list` are now **cross-platform**: they aggregate installed artifacts across every platform (deduped by name + source) instead of showing only the active `--platform`'s view. Previously, after projecting skills to (say) codex, a bare `cmx list` (defaulting to Claude) silently omitted skills that lived only in codex's `.agents/skills`. The full inventory now appears regardless of which tool each artifact is installed for.
