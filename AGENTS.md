...
- `cmx/src/local_modification.rs` — shared primitives `for_artifact`/`for_path` answering one decision, "has this installed copy been hand-edited since it was installed?" (existence guard, then lock-entry-absent-is-clean, then checksum comparison, in that order); consolidates three call sites (`install.rs`, `outdated.rs`, `info/mod.rs`) that previously applied the same guard checks in inconsistent orders
...
- `cmx/src/doctor/classify.rs` — classifies each installed artifact's state from its content checksum, records each copy's on-disk representation (portable, or generated Codex TOML) and the Codex projection checksum of a portable agent, and assembles the raw per-location rows
- `cmx/src/doctor/aggregate.rs` — folds per-location rows into logical artifacts, deciding representation-aware divergence (a Codex TOML copy is compared through the Codex projection of its portable siblings, not by raw bytes), consolidates state severity, sorts rows, and finds missing lock entries
...
The intent atlas is the externally maintained knowledge base cmf composes from and cmv verifies against: the ecosystems it supports (declared, with how to recognize each, in `ecosystems.toml` at its root), the intent records placed in the directory hierarchy that names their ecosystem, the validators beside those records, and the profiles that name a slice of it for a delivery surface (see `CMV.md`, "Three responsibilities"). This crate is the one reader of the atlas's shape that cmf and cmv share — catalog scanning and the ecosystem qualifiers a key carries, profile loading, selection, the sensors, and the compile-manifest types — so cmv does not depend on cmf and neither binary duplicates the other's reading of it. It sits beside `cmx-core` rather than inside it because cmx has no use for it and cmx-core's TypeScript twin and lockstep release would make every atlas change pay for a port and a coordinated release for a consumer that does not exist; it stays `publish = false` until a second consumer appears. It is a library, so it denies `missing_docs` at the crate root.

### intent-atlas Architecture

- `intent-atlas/src/lib.rs` — crate root; states what the atlas is and exports `catalog`, `profile`, `selection`, `sensors`, and `manifest`
...
- `intent-atlas/src/sensors.rs` — the atlas's ecosystem sensors, `ecosystems.toml` at its root: a table per ecosystem with `signatures` (predicate kinds `{ file }`, `{ file, contains }`, `{ glob }` — the extension point; an unrecognized predicate is a parse error naming the ecosystem and signature) and optional `implies`; `load` (`None` when the file is absent), `validate` against the scanned catalog (every declared name is a directory some record uses below `intents/`, every `implies` target is declared, and a declared nested ecosystem implies its parent), `load_validated` (the combination cmf and cmv call right after `catalog::scan`), `detect` (any signature holds → detected, `implies` added transitively, sorted and deduplicated; pure over the `Filesystem` gateway, reading only the project root), `Detection` (keeps "the atlas declares no sensors" apart from "nothing was detected"), and `undetected` (declared minus detected — the one comparison behind cmf's mismatch warning and cmv's `profile_mismatch`)
...
- `cmf/src/main.rs` — binary entry point; reads the atlas (catalog, then its sensors validated against that catalog), assembles, previews, and applies installs through cmx-core; at `install --local` (preview and apply) senses the current directory's ecosystems and prints the `warning: profile <id> targets <a, b> but this project shows <c, d>` line `mismatch` decides on; `status` prints `Sensors: N ecosystems` (or `none declared`)
- `cmf/src/lib.rs` — crate root; exports `assembly`, `cli`, and `mismatch`, and re-exports `intent_atlas::{catalog, profile, manifest, sensors}` so cmf's own `cmf::catalog::…` paths keep resolving (cmv must not use these re-exports)
- `cmf/src/cli.rs` — clap grammar for `assemble`, `install`, and `status`
- `cmf/src/mismatch.rs` — the pure decision behind `install --local`'s ecosystem mismatch warning: the profile's declared `[select] ecosystems` against the atlas's `Detection` at the project root; no warning when the profile declares nothing or every declared ecosystem was detected, and the `… but the atlas declares no sensors` / `… but nothing was detected` variants when there was nothing to compare against
...
- `cmv/src/pin.rs` — honouring the pinned revision: when the manifest pins a commit the checkout's `HEAD` has moved past, materialize that tree through the `ProcessRunner` gateway (`git archive` into a tar, `tar -xf` into scratch) and verify there; `--at-head` (`PinPolicy`) bypasses it; owns the `atlas` report block (`resolved_by`, pins, `verified_against`, `moved`, and `sensors` — whether the verified tree declares any)
- `cmv/src/explain.rs` — `cmv explain <intent>`: how one manifest entry resolves to a record (by `key`, by a moved record's unique `id`, not found — naming the records that share the id when that blocked the fallback), its title and status, every declared validator, which would run for the workspace's ecosystems and with exactly which argv and `--config` document (or why it is skipped, including the no-sensors reason), the stale flag, and whether the manifest dropped it; shares its resolution, stale, and argv decisions with `dispatch` and runs nothing
- `cmv/src/config.rs` — the hand-authored `cmv.toml` at the project root: `ecosystems` override, validator timeout, and per-intent tables handed to validators as JSON; a missing file means defaults
- `cmv/src/ecosystems.rs` — the ecosystems a run verifies as and where the answer came from (`Ecosystems`: `Overridden` by `cmv.toml`, `Detected` by the atlas's sensors at the project root, or `Undetectable` when the atlas declares no sensors and there is no override); serializes as the plain name list, decides which validators `admits`, and owns the reason strings — `no validator for ecosystems [..]`, the skip reason, and `atlas declares no sensors; set ecosystems in cmv.toml to override`; there is no built-in table of build files
...
- `cmv/src/report.rs` — output rendering for `check`, `status`, and `explain`: deterministic JSON (`--json`) and the human listings, including the informational `atlas has moved` line and `check`'s `profile_mismatch` (the manifest profile's ecosystems detection did not find, with its `manifest profile targets … but the workspace shows …` line); `Display` impls and their tests live here
